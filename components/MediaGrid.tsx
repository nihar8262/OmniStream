...
export function MediaGrid() {
  ...
  return (
    <section id="results-grid" className="w-full py-6 sm:py-8 px-4 sm:px-6 pb-28 sm:pb-24">
      <div className="container mx-auto max-w-5xl">
        {/* Post Metadata Card */}
        <div
          className={`mb-6 rounded-2xl border p-4 sm:p-5 backdrop-blur-xl transition-all ${
            isLinkedIn
              ? "border-sky-500/20 bg-[#0d141e]/80"
              : "border-white/[0.08] bg-[#101010]/80"
          }`}
        >
            ...
            <div className="flex items-center gap-3 min-w-0">
              <div
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full p-[1.5px] transition-all ${
                  isLinkedIn
                    ...
                    : "bg-gradient-to-tr from-[#fef3c7] via-[#e7b92f] to-[#c29017]"
                }`}
              >
                <div className="flex h-full w-full items-center justify-center rounded-full bg-[#080808] overflow-hidden">
              ...
              <div className="flex flex-col min-w-0">
                <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-wrap">
                  <span className="text-sm font-bold text-[#f5f3ed] truncate">
                    @{manifest.author?.username || (isLinkedIn ? "linkedin_user" : "instagram_user")}
                  </span>
                  {manifest.author?.fullName && (
                    <span className="text-xs text-[#a5a39c] truncate">
                      ({manifest.author.fullName})
                    </span>
                  )}
                </div>
                <span className="text-xs text-[#a5a39c]">
          ...
          {manifest.caption && (
            <div className="mt-3 pt-3 border-t border-white/[0.06]">
              <p className="text-xs text-[#a5a39c] line-clamp-2 leading-relaxed italic break-words">
                "{manifest.caption}"
              </p>
            </div>
          )}
        ...
        <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-white/[0.06] bg-[#101010]/60 p-3 sm:p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 sm:gap-3">
              ...
              >
                {allSelected ? (
                  <>
                    <Square className="h-3.5 w-3.5 text-[#a5a39c]" />
              ...
              <span className="text-xs text-[#a5a39c] font-medium">
                {t("selectedCount", {
            ...
            {manifest.itemCount > 20 && (
              <div className="flex items-center gap-1.5 text-[11px] text-[#e7b92f]/90 w-full sm:w-auto">
          ...
          {totalBatches > 1 && (
            <div className="pt-2.5 border-t border-white/[0.06] flex flex-wrap items-center gap-2">
              <span className="text-xs text-[#a5a39c] font-medium mr-1">
                Select by batch:
              </span>
                    ...
                    className={`h-7 px-2.5 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                      isBatchSelected
                        ? isLinkedIn
                          ...
                          : "bg-[#e7b92f]/20 border-[#e7b92f] text-[#fef3c7] font-bold"
                        : "border-white/[0.08] text-[#a5a39c] hover:bg-white/[0.06] hover:text-[#f5f3ed]"
                    }`}
                  >
